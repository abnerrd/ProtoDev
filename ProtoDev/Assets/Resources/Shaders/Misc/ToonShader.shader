...
//	This is a great shader, but these guys sound so weird. https://www.youtube.com/watch?v=3qBDTh9zWrQ
...
Shader "Custom/ToonShader" 
{
		...
		SubShader
	{
		Pass
		{
			...
			#pragma vertex vert				//	tells the cg to sue a vertex-shader called vert
			...
			vertexOutput vert(vertexInput input)
			{
				...
				//	world position
				float4 ourWorldPosition = mul(unity_ObjectToWorld, input.vertex);
					
				//	view direction		//	vector from object to the camera
				output.viewDir = normalize(_WorldSpaceCameraPos.xyz - ourWorldPosition.xyz);	

				//	light direction
				float3 fragmentToLightSource = (_WorldSpaceCameraPos.xyz - ourWorldPosition.xyz);
			...
			float4 frag(vertexOutput input) : COLOR
			{
				...
				return float4(combinedLight, 1.0);	//	+ tex2D(_MainTex, input.uv);	//	+ ENABLES TEXTURE TOON SHADING
			}
